...
local function InitializeUI()
    ...
    -- 设置说明文本
    infoText:SetText(
        "关注规则:\n1. Global频道: 仅包含关注词的发言显示\n2. 大喊频道: 仅同阵营包含关注词的发言显示\n3. 关注词高亮: 高亮显示关注词")
    ...
    configFrame:SetScript("OnShow", function()
        ...
        if playerLanguage then
            local factionName = playerLanguage
            factionText:SetText(playerLanguage)
        else
...
local function SetupBlizzardOptions()
    ...
    cfgFrame.name = "LFGC - 关注词白名单"

    -- Create an option button in the center of the frame to launch MSBT's options.
    local button = CreateFrame("Button", nil, cfgFrame, "OptionsButtonTemplate")
